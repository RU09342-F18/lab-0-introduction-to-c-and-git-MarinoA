...
/: Division. Divides num1 by num2. Inputs support all whole numbers and produces a whole number as a result. Num2 cannot be zero, but the program will not allow a divide by zero. Answers which result in a fraction will be truncated to the 1’s place. (Example: 20/6 will result in an answer of 3) 
%: Modulus. Performs a modulo operation, finding the remainder when num1 is divided by num2. Inputs support all whole numbers and produces a whole number as a result. Num2 cannot be zero, but the program will not allow a divide by zero.(Example: 10%3 = 1) 
